...
public class MinCostClimbDP {
    ...
    public int minCostClimbingStairsRecursive(int[] cost) {
        ...
        int[] newArr1 = Arrays.copyOfRange(cost, 1, cost.length);
        int[] newArr2 = Arrays.copyOfRange(cost, 2, cost.length);
        return Integer.min(cost[0] + minCostClimbingStairs(newArr1), cost[1] + minCostClimbingStairs(newArr2));
    }
    ...
    public int minCostClimbingStairs(int[] cost) {
        ...
        Arrays.fill(dp, -1);
        return minCostOptimized(cost, dp, 0, cost.length);
    }

    public int minCostOptimized(int[] cost, int[] dp, int i, int n) {
        if (dp[i] != -1)
            return dp[i];
        if ((n - i) == 0 || (n - i) == 1)
            return 0;
        else if ((n - i) == 2)
            return Integer.min(cost[i], cost[i + 1]);
        dp[i] = Integer.min(cost[i] + minCostOptimized(cost, dp, i + 1, n), cost[i + 1] + minCostOptimized(cost, dp, i + 2, n));
        return dp[i];
    }
